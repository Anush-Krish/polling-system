...
class SnapService {
  ...
  async uploadSnap(snapData) {
    try {
      ...
      if (snapData.imageUrl.startsWith('data:')) {
        console.log('Attempting to upload media to R2...');

        // Determine if it's an image or video
        const isVideo = snapData.imageUrl.startsWith('data:video');
        const extension = isVideo ? 'mp4' : 'jpg';
        const fileName = isVideo ? 'snap.mp4' : 'snap.jpg';

        // Convert data URL to buffer
        const base64Data = snapData.imageUrl.replace(/^data:(image|video)\/\w+;base64,/, '');
        const mediaBuffer = Buffer.from(base64Data, 'base64');

        // Upload to R2
        const uploadResult = await uploadImageToR2(mediaBuffer, fileName);
        if (!uploadResult.success) {
          console.error('R2 upload failed:', uploadResult.error);
          throw new Error(`Failed to upload media to R2: ${uploadResult.error}`);
        }
